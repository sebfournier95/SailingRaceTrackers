...
const geovoileHostname = 'tracking2024.vendeeglobe.org';
const resourcesBasePath = '/data/versions.txt?v';
const trackerBasePath = '/data/';
// ================================== UTILS ===================================
...
let boatsPath = trackerBasePath + 'tracker_config.hwx?v=' + boatsVersion;

downloadReport(boatsPath, function (reportData) {
	fs.writeFile('./boats-VG.json', reportData, err => {
...
getTracksVersion(function (tracksVersion) {
	let tracksPath = trackerBasePath + 'tracker_tracks.hwx?v=' + tracksVersion;
	downloadReport(tracksPath, function (reportData) {
		fs.writeFile('./tracks-VG.json', reportData, err => {
			if (err) { handleError(err) }
		});
	});
});